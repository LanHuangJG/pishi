...
class ReflectUtils {
    ...
    static List<CtClass> toCtClasses(Collection<File> classPaths, ClassPool classPool) {
        List<String> classNames = new ArrayList<>()
        List<CtClass> allClass = new ArrayList<>();
        def startTime = System.currentTimeMillis()
        classPaths.each { file ->
            classPool.insertClassPath(file.absolutePath)
            if (file.isDirectory()) {
                def dirPath = file.absolutePath
                FileUtils.listFiles(file, null, true).each {
                    if (it.absolutePath.endsWith('.class')) {
                        def className = it.absolutePath.substring(dirPath.length() + 1, it.absolutePath.length() - '.class'.length()).replaceAll(Matcher.quoteReplacement(File.separator), '.')
