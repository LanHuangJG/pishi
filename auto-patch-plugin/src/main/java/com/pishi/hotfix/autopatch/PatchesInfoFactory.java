...
import static com.pishi.hotfix.autopatch.Config.classPool;
...
public class PatchesInfoFactory {
    ...
    private CtClass createPatchesInfoClass() {
        try {
            ...
            ctPatchesInfoImpl.setInterfaces(new CtClass[]{classPool.get("com.pishi.hotfix.PatchesInfo")});
            ...
            for (int i = 0; i < Config.modifiedClassNameList.size(); i++) {
                if (Constants.OBSCURE) {
                    methodBody.append("com.pishi.hotfix.PatchedClassInfo patchedClass" + i + " = new com.pishi.hotfix.PatchedClassInfo(\"" + ReadMapping.getInstance().getClassMappingOrDefault(Config.modifiedClassNameList.get(i)).getValueName() + "\",\"" + NameManger.getInstance().getPatchControlName(Config.modifiedClassNameList.get(i).substring(Config.modifiedClassNameList.get(i).lastIndexOf('.') + 1)) + "\");");
                } else {
                    methodBody.append("com.pishi.hotfix.PatchedClassInfo patchedClass" + i + " = new com.pishi.hotfix.PatchedClassInfo(\"" + Config.modifiedClassNameList.get(i) + "\",\"" + NameManger.getInstance().getPatchControlName(Config.modifiedClassNameList.get(i).substring(Config.modifiedClassNameList.get(i).lastIndexOf('.') + 1)) + "\");");
                }
